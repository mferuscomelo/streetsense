...
|---|---|---|
| [Sensirion I2C SEN5X](https://github.com/Sensirion/arduino-i2c-sen5x) `0.3.0` | BSD-3-Clause | Sensirion's own driver for the Sensirion SEN54. Hand-rolling the I2C command set and its CRC-8 would be a worse engineering decision and an untestable one, since this project has no way to verify a hand-written driver except against the same hardware the official one already supports. |
| Sensirion Core `0.7.3` | BSD-3-Clause | Transitive dependency of the SEN5X driver. |
| [Adafruit MAX1704X](https://github.com/adafruit/Adafruit_MAX1704X) `1.0.3` | BSD | Adafruit's own driver for the MAX17048 fuel gauge on their 5580 breakout, for the same reason as the SEN5X driver above. |
