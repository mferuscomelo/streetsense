...
- **The SEN54 is a consumer PM/VOC *proxy*, not a lab-grade reference
  ...
  BLE to the app. `ledglasses` (the default environment) still runs a
  synthetic source for demos without the sensor attached; see
  `docs/ble-protocol.md` for how synthetic data is flagged end-to-end.
...
- **Classification is rule-based, over sensor cross-products, not a
  ...
  - The rolling per-`(cell, hour)` baseline (Stream Gatherers computing an
    EWMA-smoothed mean/stddev per pollutant) is the same mechanism this
    project has shipped from the start.
  - The event classifier (`backend/.../anomaly/AnomalyDetector.java`) reads
    the *combination* of which channels rose (particulates alone,
...
- **Seeded contributors are always flagged, never presented as measured.**
  ...
  `seed:` prefix and every seeded reading sets the synthetic-data flag, so
...
- **Synthetic data is always flagged.** `FLAG_MOCK_DATA` on the wire, a
