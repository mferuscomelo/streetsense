...
values drift, the app will simply never find the node, so get them from that
...
|---|---|---|---|---|
| 0 | `version` | u8 | n/a | schema version, currently `2` |
| 1 | `flags` | u8 | n/a | bit0 = `FLAG_MOCK_DATA` (flags a synthetic reading), bit1 = `FLAG_CHARGING`, bit2 = `FLAG_BATTERY_VALID` |
| 2 | `seq` | u16 | n/a | wraps; detects dropped notifications |
| 4 | `pm1` | u16 | ÷10 | ug/m³ |
| 6 | `pm2_5` | u16 | ÷10 | ug/m³ |
| 8 | `pm4` | u16 | ÷10 | ug/m³ |
| 10 | `pm10` | u16 | ÷10 | ug/m³ |
...
| 20 | `batt_mv` | u16 | n/a | millivolts; 0 when `FLAG_BATTERY_VALID` is clear |
| 22 | `batt_soc` | u16 | ÷10 | battery percent, 0 to 100; 0 when invalid |
...
backend has moved on to understanding v2. The app's own subset decoder
...
`FLAG_MOCK_DATA` is set on every packet produced by the firmware's synthetic
sensor source, `firmware/lib/mock_sensor_source/mock_sensor_source.cpp`, used
for demos without a physical sensor attached. The app surfaces this as a
badge in the UI, and the backend passes a `mock` field through every API
response. Neither is optional: no judge or user should be able to mistake a
synthetic reading for a real one. Battery telemetry is real on both firmware
environments regardless of this flag; the physical MAX17048 is composed onto
the reading independently of which environmental sensor source, synthetic or
SEN54, produced the rest of the packet.
